...
constexpr auto Clamp(A a, B b, C c) -> typename std::common_type<A, B, C>::type {
  using Result = typename std::common_type<A, B, C>::type;
  const Result minCandidate = static_cast<Result>(a);
  const Result valueCandidate = static_cast<Result>(b);
  const Result maxCandidate = static_cast<Result>(c);
  return std::min(std::max(minCandidate, valueCandidate), maxCandidate);
}
