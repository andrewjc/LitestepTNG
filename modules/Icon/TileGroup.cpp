...
void TileGroup::SetFolder(LPWSTR folder) {
  ...
  {
    hr = SHGetKnownFolderIDList(FOLDERID_Desktop, 0, nullptr, &idList);
    if (SUCCEEDED(hr))
    {
      hr = SHGetDesktopFolder(reinterpret_cast<IShellFolder**>(&mWorkingFolder));
    }
  }
  else
  {
    hr = SHParseDisplayName(folder, nullptr, &idList, 0, nullptr);
    if (SUCCEEDED(hr))
    {
      IShellFolder* parent = nullptr;
      PCITEMID_CHILD child = nullptr;
      if (SUCCEEDED(SHBindToParent(idList, IID_IShellFolder, reinterpret_cast<void**>(&parent), &child)))
      {
        hr = parent->BindToObject(child, nullptr, IID_IShellFolder, reinterpret_cast<void**>(&mWorkingFolder));
        parent->Release();
      }
      else
      {
...
void TileGroup::AddIcon(PCITEMID_CHILD pidl) {
  ...
  request.folder = mWorkingFolder;
  request.targetIconWidth = mTileSettings.mIconSize;
    request.id = static_cast<PITEMID_CHILD>(ILClone(pidl));
...
HRESULT TileGroup::GetFolderPath(LPWSTR buf, UINT cchBuf) const {
  ...
  PIDLIST_ABSOLUTE curFolder = nullptr;
  HRESULT hr = E_FAIL;
  ...
  if (SUCCEEDED(hr = mWorkingFolder->QueryInterface(IID_IPersistFolder2, (LPVOID*) &ipsf2))) {
    ...
    if (SUCCEEDED(hr)) {
      if (!SHGetPathFromIDListW(curFolder, buf))
      {
        hr = E_FAIL;
      }
    }
...
void TileGroup::ContextMenu() {
  ...
  HMENU menu;
  std::vector<PCITEMID_CHILD> items;
      HRESULT hr;
