...
namespace {
    ...
    bool HasVisibleOwner(HWND hWnd) {
        HWND hwndWalk = GetAncestor(hWnd, GA_ROOTOWNER);
        HWND hwndTry;
        while ((hwndTry = GetLastActivePopup(hwndWalk)) != hwndWalk) {
            if (IsWindowVisible(hwndTry) && !IsWindowCloaked(hwndTry)) {
                break;
            }
            hwndWalk = hwndTry;
        }
    ...
    bool IsKnownShellWindow(HWND hWnd) {
        WCHAR className[64];
    ...
    bool TryGetProcessImageName(HANDLE process, std::wstring &imagePath)
    {
        typedef BOOL (WINAPI *QueryFullProcessImageNameWFunc)(HANDLE, DWORD, LPWSTR, PDWORD);
        static QueryFullProcessImageNameWFunc queryFullProcessImageNameW =
            reinterpret_cast<QueryFullProcessImageNameWFunc>(GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "QueryFullProcessImageNameW"));

        WCHAR buffer[MAX_PATH];
        DWORD size = _countof(buffer);

        if (queryFullProcessImageNameW != nullptr && queryFullProcessImageNameW(process, 0, buffer, &size))
        {
            buffer[_countof(buffer) - 1] = L'\0';
            imagePath.assign(buffer);
            return true;
        }

        typedef DWORD (WINAPI *GetModuleFileNameExWFunc)(HANDLE, HMODULE, LPWSTR, DWORD);
        static HMODULE psapiModule = nullptr;
        static GetModuleFileNameExWFunc getModuleFileNameExW = nullptr;

        if (psapiModule == nullptr)
        {
            psapiModule = GetModuleHandleW(L"Psapi.dll");
            if (psapiModule == nullptr)
            {
                psapiModule = LoadLibraryW(L"Psapi.dll");
            }
            if (psapiModule != nullptr)
            {
                getModuleFileNameExW = reinterpret_cast<GetModuleFileNameExWFunc>(GetProcAddress(psapiModule, "GetModuleFileNameExW"));
            }
        }

        if (getModuleFileNameExW != nullptr && getModuleFileNameExW(process, nullptr, buffer, _countof(buffer)) > 0)
        {
            buffer[_countof(buffer) - 1] = L'\0';
            imagePath.assign(buffer);
            return true;
    ...
    bool HasBlockedWindowTitle(HWND hWnd)
    {
        WCHAR title[256];
        ...
        std::wstring lowered(title, title + length);
        std::transform(lowered.begin(), lowered.end(), lowered.begin(), ::towlower);

        static const wchar_t* blockedFragments[] = {
            L"windows shell experience",
            L"windows input experience",
        };

        for (const wchar_t* fragment : blockedFragments)
        {
            if (lowered.find(fragment) != std::wstring::npos)
            {
                return true;
            }
        }
    ...
    bool HasBlockedProcessName(HWND hWnd)
    {
        DWORD processId = 0;
        ...
        bool blocked = false;
        if (TryGetProcessImageName(process, imagePath))
        {
            size_t pos = imagePath.find_last_of(L"\\/");
            std::wstring fileName = (pos == std::wstring::npos) ? imagePath : imagePath.substr(pos + 1);
            std::transform(fileName.begin(), fileName.end(), fileName.begin(), ::towlower);

            static const wchar_t* blockedProcesses[] = {
                L"shellexperiencehost.exe",
                L"textinputhost.exe",
            };

            for (const wchar_t* blockedProcess : blockedProcesses)
            {
                if (fileName == blockedProcess)
                {
                    blocked = true;
                    break;
                }
...
void WindowManager::Start()
{
    ...
    // Schedule periodic maintenance to keep buttons and overlays in sync.
...
bool WindowManager::IsTaskbarWindow(HWND hWnd)
{
    ...
    if (IsKnownShellWindow(hWnd) || HasVisibleOwner(hWnd))
    {
        return false;
    }

    if (HasBlockedWindowTitle(hWnd) || HasBlockedProcessName(hWnd))
    {
        return false;
    }
