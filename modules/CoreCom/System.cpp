...
HRESULT nCore::System::_Init(HMODULE hCoreInstance) {
  INIT_FUNC(FetchMonitorInfo);
    ...
    FUNC_VAR_NAME(LoadFolder) = reinterpret_cast<decltype(FUNC_VAR_NAME(LoadFolder))>(GetProcAddress(hCoreInstance, "NCore_LoadFolder"));
  if (FUNC_VAR_NAME(LoadFolder) == nullptr) return E_NCORECOM_FUNC_INIT;
    FUNC_VAR_NAME(LoadFolderItem) = reinterpret_cast<decltype(FUNC_VAR_NAME(LoadFolderItem))>(GetProcAddress(hCoreInstance, "NCore_LoadFolderItem"));
  if (FUNC_VAR_NAME(LoadFolderItem) == nullptr) return E_NCORECOM_FUNC_INIT;
    FUNC_VAR_NAME(CancelLoad) = reinterpret_cast<decltype(FUNC_VAR_NAME(CancelLoad))>(GetProcAddress(hCoreInstance, "NCore_CancelLoad"));
  if (FUNC_VAR_NAME(CancelLoad) == nullptr) return E_NCORECOM_FUNC_INIT;
...
void nCore::System::_DeInit() {
  ...
  FUNC_VAR_NAME(CancelLoad) = nullptr;
...
void nCore::CancelLoad(UINT64 id) {
  ASSERT(nCore::Initialized());
  FUNC_VAR_NAME(CancelLoad)(id);
}
